...
from tuneavideo.models.unet_meme import UNet3DConditionModel
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    kwargs = config.get("unet_additional_kwargs", {
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % validation_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        if validation_data.get("use_inv_latent", False):  
                        ...
                        for idx, prompt in enumerate(validation_data["prompts"]):
                            ...
                            save_videos_grid(sample, f"{output_dir}/samples/sample_{global_step}/{prompt}.gif", loop=0)
                            samples.append(sample)
                        samples = torch.concat(samples)
                        save_path = f"{output_dir}/samples/sample_{global_step}.gif"
...
if __name__ == "__main__":
    ...
    config = OmegaConf.to_container(OmegaConf.load("configs/cat-pop-meme.yaml"), resolve=True)
